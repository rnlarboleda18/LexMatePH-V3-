...
# Helper removed - using shared utils.clerk_auth
    ...
    try:
        with get_db() as conn:
            with conn.cursor() as cur:
                ...
                if not cur.fetchone(): return func.HttpResponse(json.dumps({"error": "Not found or unauthorized"}), status_code=404)
                
                cur.execute("SELECT id, content_id, content_type, code_id, title, subtitle, sort_order FROM playlist_items WHERE playlist_id = %s ORDER BY sort_order ASC", (playlist_id,))
                rows = cur.fetchall()
                items = []
                for r in rows:
                    items.append({
    ...
    try:
        with get_db() as conn:
            with conn.cursor() as cur:
                ...
                cur.execute("DELETE FROM playlist_items WHERE id = %s AND playlist_id = %s RETURNING id", (item_id, playlist_id))
                if not cur.fetchone(): return func.HttpResponse(json.dumps({"error": "Item not found"}), status_code=404)
                conn.commit()
        return func.HttpResponse(json.dumps({"success": True}), mimetype="application/json", status_code=200)
    except Exception as e:
            ...
            with conn.cursor() as cur:
                ...
                for item in items_to_add:
                    ...
                    cur.execute("""
                        INSERT INTO playlist_items (playlist_id, content_id, content_type, code_id, title, subtitle, sort_order)
                        VALUES (%s, %s, %s, %s, %s, %s, %s) RETURNING id
                    """, (playlist_id, str(c_id), c_type, item.get('code_id'), item.get('title'), item.get('subtitle'), next_order))
                    new_id = cur.fetchone()[0]
                ...
                conn.commit()
        return func.HttpResponse(json.dumps(inserted_items), mimetype="application/json", status_code=201)
    except Exception as e:
