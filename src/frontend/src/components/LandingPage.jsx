...
/**
 * Minimal marketing landing: install steps + hero only (no header tile, no feature grids).
 */
const LandingPage = ({ isDarkMode, toggleTheme, onEnterApp }) => {
    return (
        <div className="landing-page min-h-screen bg-transparent font-sans text-gray-900 dark:text-gray-100">
            ...
            <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-4 pb-20 pt-[max(4.5rem,env(safe-area-inset-top))] sm:gap-8 sm:px-6 sm:pb-24 lg:px-8">
                <section id="install" className="scroll-mt-8">
                    <div className="rounded-2xl border border-slate-200/90 bg-white/75 p-6 shadow-sm backdrop-blur-md dark:border-white/10 dark:bg-slate-950/50 sm:p-8">
                        <h2 className="font-display text-xl font-semibold tracking-tight text-slate-900 dark:text-white sm:text-2xl">
                            Install in three steps
                        </h2>
                        <p className="mt-1 max-w-2xl text-sm text-slate-600 dark:text-slate-400">
                            Add LexMatePH to your home screen for a focused, app-like experience.
                        </p>
                        <ol className="mt-6 grid list-none gap-4 pl-0 sm:grid-cols-3 sm:gap-5">
                            {INSTALL_STEPS.map(({ step, title, body, Icon }) => (
                                <li
                                    key={step}
                                    className="flex flex-col rounded-xl border border-slate-200/80 bg-white/90 p-5 shadow-[0_1px_2px_rgba(15,23,42,0.04)] dark:border-white/10 dark:bg-slate-900/40 sm:p-6"
                                >
                                    <div
                                        className="mb-4 flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br from-violet-600 to-indigo-700 text-white shadow-md shadow-indigo-900/20"
                                        aria-hidden
                                    >
                                    ...
                                    <h3 className="mt-1.5 font-display text-base font-semibold leading-snug text-slate-900 dark:text-white">
                                        {title}
                                    </h3>
                ...
                <section className="relative min-w-0">
                    ...
                    />
                    <div
                        className={`${LG_HERO} relative w-full min-w-0 max-w-full px-6 py-[calc(1.5rem-0.5cm)] sm:px-8 sm:py-[calc(2rem-0.5cm)] lg:grid lg:grid-cols-[minmax(0,0.82fr)_minmax(0,1.18fr)] lg:items-center lg:gap-8 lg:px-6 lg:py-[calc(1.5rem-0.5cm)] xl:grid-cols-[minmax(0,0.78fr)_minmax(0,1.22fr)] xl:px-7 xl:py-[calc(1.75rem-0.5cm)]`}
                    >
                        <div
