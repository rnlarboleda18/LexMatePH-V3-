...
print("EXTRACTING PHASE 2 ULTRA-LARGE CASES (500K-1M)")
print("=" * 50)

# Get the 17 largest Phase 2 cases
cur.execute("""
    SELECT id
...
for i in range(num_workers):
    ...
    if batch:
        filename = f"phase2_ultralarge_{i+1:02d}.txt"
        with open(filename, 'w') as f:
            for case_id in batch:
                f.write(f"{case_id}\n")
        print(f"Created {filename}: {len(batch)} cases")

print(f"\nReady to launch 2 workers with gemini-3-pro-preview")
conn.close()
